//make sure all images loaded before trying to add popup button
...
function Initialize()
{
    ...
    {
        //skip adding popup if it already exists
        if ( $( this ).parent().hasClass( "gpImgBorder" ) )
            return true;
        ...
        if ( this.naturalWidth > minWidth )
        {
            ...
            qsButton.on( "click", { url: imageURL },
                function( eventObject )
                {
                    ...
                    var fname = ( fileSavePath + ( fileSavePath.length == 0 ? "" : "\\" ) + eventObject.data.url.substr( index ).replace( /[^a-z0-9._-]/gi, '_' ) ).trim();
...
browser.runtime.onMessage.addListener(
    function( request, sender, sendResponse )
    {
        ...
        {
            console.log( "refresh" );
            //only try to add popup again if it doesn't already exist
